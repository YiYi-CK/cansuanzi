<template>
  <div>
    ...
    <n-space justify="space-between" style="margin-bottom: 16px">
      ...
      <n-button type="primary" @click="showAdd = true">{{ t('employee.add_employee') }}</n-button>
    </n-space>
    ...
    <!-- 新增/编辑弹窗 -->
    <n-modal v-model:show="showAdd" :title="editing ? t('employee.edit_employee') : t('employee.add_employee')" style="width: 500px">
...
const form = ref({ name: '', email: '', phone: '', employment_type: 'casual', base_hourly_rate: 0, password: '' });
const typeOptions = [
...
function editEmployee(emp) {
  editing.value = emp;
  form.value = { name: emp.name, email: emp.email, phone: emp.phone || '', employment_type: emp.employment_type, base_hourly_rate: parseFloat(emp.base_hourly_rate), password: '' };
  showAdd.value = true;
}
...
async function saveEmployee() {
  ...
  try {
    ...
    showAdd.value = false;
    editing.value = null;
    form.value = { name: '', email: '', phone: '', employment_type: 'casual', base_hourly_rate: 0, password: '' };
    fetchEmployees();
  } catch (err) {
